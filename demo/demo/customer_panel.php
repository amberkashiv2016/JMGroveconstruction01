...
{
 $sql_user = "update dbo.new_customer set Password='".$_REQUEST['password_popup']."' where id='".$customerID."'";
 $query_user = sqlsrv_query($conn, $sql_user);
    ...
    <div class="user_panel">
        ...
        <ul>
            ...
            <div class="content_panel col-md-12">
				...
				<?php
				$i=0;
				while ($row_job = sqlsrv_fetch_array($query_job))
				{ ?> 
				<tr style="color:#FF0000;"><td><?php if($_REQUEST['pay_submit']=='' && ($total-1)>$paid )  { ?>
					
					<?php }?>
					
					<?php
					 $sql_date = "SELECT * FROM jgrov_User.tblTransactionDetails where tblSEId='".$row_job['EstimateId']."'";
					 $query_date = sqlsrv_query($conn, $sql_date);
					 $row_date1 = sqlsrv_fetch_array($query_date);
					 $row_date2 = sqlsrv_fetch_array($query_date);
					 $row_date3 = sqlsrv_fetch_array($query_date);
					//parse quote number
					$pos=strpos($row_job['SoldJobId'],"-"); 
					
					$id=substr($row_job['SoldJobId'],1,$pos);
					//echo "id=".$id;
					$quote_num=substr($row_job['SoldJobId'],$pos+1,strlen($row_job['SoldJobId']));
					//echo "quote_num=".$quote_num;
					//echo "length=".strlen($row_job['SoldJobId'];
					$sql_paid ="SELECT sum(TotalPaid) as paid FROM jgrov_User.tblTransactionDetails where tblSEId = '".$row_job['EstimateId']."'";
	
							$query_paid = sqlsrv_query($conn, $sql_paid);
							$row_paid = sqlsrv_fetch_array($query_paid);
							$paid=$row_paid['paid'];
							
							$sql_total ="SELECT Totalamt as total FROM jgrov_User.tblTransactionDetails where tblSEId = '".$row_job['EstimateId']."'";
							$query_total = sqlsrv_query($conn, $sql_total);
							$row_total = sqlsrv_fetch_array($query_total);
							$total=$row_total['total'];
					if($row_job['InvoicePath']=='')
					{
					if($_REQUEST['pay_submit']=='' && ($total-1)>$paid && $row_date3=='' )  { ?>
					<input type="radio"  name="payment_id" id="<?php echo $row_job['EstimateId'];?>" class="click_class1">
					
					<?php }echo $quote_num;// echo "paid=".$paid."total=".$total;
					 }
					else{
					?>
					
					<?php if($_REQUEST['pay_submit']=='' && ($total-1)>$paid && $row_date3=='')  { ?>
					<input type="radio"  name="payment_id" id="<?php echo $row_job['EstimateId'];?>" class="click_class1">
					<?php }?>
					<a href="CustomerDocs/Pdfs/<?php echo $row_job['InvoicePath']; ?>" target="_blank" style="color:#FF0000;" ><?php echo $quote_num; ?> </a>
					<?php }?>
					<input type="hidden" name="amount_hidden_sold<?php echo $row_job['EstimateId'];?>" value="<?php echo number_format((float)$row_job['TotalPrice']/3,2,'.','');?>" id="amount_hidden_sold<?php echo $row_job['EstimateId'];?>" ></td>
					<td >$ <?php echo $row_job['TotalPrice'];?></td>
					<td >$ <?php  echo number_format((float)$row_job['TotalPrice']/3,2,'.','');echo "<br>"; if($row_date1['TransDate']){echo $row_date1['TransDate']->format('d-F-Y H:i:s'); echo "  <label style='color:black;'>Debit/Cash</label>";} // echo $row_job['TotalPrice']/3;?></td>
					<td >$ <?php echo number_format((float)$row_job['TotalPrice']/3,2,'.','');echo "<br>"; 
					if($row_date1['TransDate']){
					if($row_date1['Totalamt']<=($row_date1['TotalPaid']))
					{echo $row_date1['TransDate']->format('d-F-Y H:i:s'); echo "  <label style='color:black;'>Debit/Cash</label>";}
					}
					if($row_date2['TransDate']){echo $row_date2['TransDate']->format('d-F-Y H:i:s'); echo "  <label style='color:black;'>Debit/Cash</label>";} //echo $row_job['TotalPrice']/3;?></td>
					<td >$ <?php echo number_format((float)$row_job['TotalPrice']/3,2,'.','');echo "<br>"; 
					if($row_date1['TransDate']){
					if($row_date1['Totalamt']<=($row_date1['TotalPaid']))
					{echo $row_date1['TransDate']->format('d-F-Y H:i:s'); echo "  <label style='color:black;'>Debit/Cash</label>";}
					...
					if($row_date3['TransDate']){echo $row_date3['TransDate']->format('d-F-Y H:i:s'); echo "  <label style='color:black;'>Debit/Cash</label>";}
					else if(($total)<=$paid){
					if($row_date2['TransDate']){echo $row_date2['TransDate']->format('d-F-Y H:i:s'); echo "  <label style='color:black;'>Debit/Cash</label>";} }
					//echo $row_job['TotalPrice']/3;?></td>
					<td >-</td>
					<td >-</td></tr>
				<?php }
				
				while ($row_job = sqlsrv_fetch_array($query_job_custom))
				{ ?> 
				<tr style="color:#FF0000;"><td><?php if($_REQUEST['pay_submit']=='' && ($total-1)>$paid )  { ?>
					...
					<a href="CustomerDocs/Pdfs/<?php echo $row_job['InvoicePath']; ?>" target="_blank" style="color:#FF0000;" ><?php echo $quote_num; ?> </a>
					<?php }?>
					<input type="hidden" name="amount_hidden_sold<?php echo $row_job['EstimateId'];?>" value="<?php echo number_format((float)$row_job['ProposalCost']/3,2,'.','');?>" id="amount_hidden_sold<?php echo $row_job['EstimateId'];?>" ></td>
					<td >$ <?php echo $row_job['ProposalCost'];?></td>
					<td >$ <?php  echo number_format((float)$row_job['ProposalCost']/3,2,'.','');echo "<br>"; if($row_date1['TransDate']){echo $row_date1['TransDate']->format('d-F-Y H:i:s'); echo "  <label style='color:black;'>Debit/Cash</label>";} // echo $row_job['ProposalCost']/3;?></td>
					<td >$ <?php echo number_format((float)$row_job['ProposalCost']/3,2,'.','');echo "<br>"; 
					if($row_date1['TransDate']){
					if($row_date1['Totalamt']<=($row_date1['TotalPaid']))
					{echo $row_date1['TransDate']->format('d-F-Y H:i:s'); echo "  <label style='color:black;'>Debit/Cash</label>";}
					}
					if($row_date2['TransDate']){echo $row_date2['TransDate']->format('d-F-Y H:i:s'); echo "  <label style='color:black;'>Debit/Cash</label>";} //echo $row_job['ProposalCost']/3;?></td>
					<td >$ <?php echo number_format((float)$row_job['ProposalCost']/3,2,'.','');echo "<br>"; 
					if($row_date1['TransDate']){
					if($row_date1['Totalamt']<=($row_date1['TotalPaid']))
					{echo $row_date1['TransDate']->format('d-F-Y H:i:s'); echo "  <label style='color:black;'>Debit/Cash</label>";}
				...
				<?php $i++; } ?>
					...
					$sql_path ="select * FROM dbo.tblcustomer_followup where EstimateId='".$row['EstimateId']."' order by Id DESC ";
					$query_path = sqlsrv_query($conn, $sql_path);
					$row_path = sqlsrv_fetch_array($query_path);
				...
				<?php $i++; } ?>
					...
					<tr><td>First Name</td><td><input type="text" name="f_name" class="form_style" id="f_name" value="<?php echo $row_user['CustomerName'];?>"></td><td></td><td></td>
					<td>Last Name</td><td><input type="text" name="l_name"  class="form_style" id="l_name" value="<?php echo $row_user['lastname'];?>"></td></tr>
					<tr><td style="text-align:left;vertical-align:top;padding:0">Address</td><td style="text-align:left;vertical-align:top;padding:0"><textarea name="address" style="width:190px;" id="address" value=""><?php echo $row_user['CustomerAddress'];?></textarea></td><td></td><td></td>
					<td style="text-align:left;vertical-align:top;padding:0">Phone</td><td>
					<div id="addr_div1" >  
														 <div >  
					...
					<tr><td style="text-align:left;vertical-align:top;padding:0">Zip</td><td style="text-align:left;vertical-align:top;padding:0"><input type="text" name="zip" maxlength="10" id="zip" value="<?php echo $row_user['ZipCode'];?>" class="form_style num"></td><td></td><td></td>
					...
					<td  style="text-align:left;vertical-align:top;padding:0">
														...
														<div id="addr_div">
														 ...
														 ?>
														  <div >  <?php if($j=='0'){echo 'Add Email:';}?><input type="text" name="email<?=($j + 2)?>" id="email<?=($j + 2)?>" value="<?php echo $allemails[$j];?>" class="form_style"><br> </div>
													 </div>
													 <?php }?>
																	
					
					</td></tr>
					<tr><td>City:</td><td><input type="text" name="city" id="city" value="<?php echo $row_user['City'];?>" class="form_style"></td><td></td><td></td></tr>
					<tr><td>State:</td><td><input class="form_style" type="text" name="state" id="state" value="<?php echo $row_user['State'];?>"></td><td></td><td></td><td>Personal/Business:</td><td><select name="perbis" id="perbis" class="form_style">
					...
					<p style="padding: 0px 15px 10px 15px;">
						...
						<div id="address_form">
						<table style="margin:20px;margin-left:40px;">
					<tr><td>Address</td><td><textarea name="address1" id="address1" style="width:190px;" value=""><?php echo $row_user['BillingAddress'];?></textarea></td></tr>
					<tr><td>Zip</td><td><input type="text" name="zip1" id="zip1" value="<?php echo $row_user['ZipCode'];?>" class="form_style num"></td></tr>
					<tr><td>City:</td><td><input type="text" name="city1" id="city1" value="<?php echo $row_user['City'];?>" class="form_style"></td></tr>
					<tr><td>State:</td><td><input type="text" name="state1" id="state1" value="<?php echo $row_user['State'];?>" class="form_style"></td></tr>
					<tr><td></td><td></td></tr>
					</table>
						</div>
					</p>
					...
					<p style="padding: 5px 15px 10px 45px;font-weight:800;font-size:20px;color:#c72121;" align="left">
						...
						<?php if($_REQUEST['pay_submit']!='') {echo "Debit/Cash";}
						else{?>
					...
					echo '<input type="hidden" name="total_price" id="total_price"  value="'.($row['TotalPrice']).'"  readonly="true" >';
					 ...
					 ?>
						...
						<tr><td style="text-align:left;vertical-align:top;padding:0">Bank:</td><td style="text-align:left;vertical-align:top;padding:0">
						<input type="text" class="form_style" name="bank" id="bank" >
</td>
						<td style="text-align:left;vertical-align:top;padding:0">Last 4 SSN:</td><td style="text-align:left;vertical-align:top;padding:0"><input type="password" class="form_style num" name="ssn" id="ssn" maxlength="4" ><i class="fa fa-question-circle" style="cursor:pointer;" title=" Social Security Number is given by Social Security Administration . Every town or a city has a Social Security Office. You can find out about the nearest Social Security office by visiting their web site
or by calling their toll-free number 1-800-772-1213. "></i></td>
						<td style="text-align:left;vertical-align:top;padding:0"></td><td style="text-align:left;vertical-align:top;padding:0"><input checked="checked" type="radio" name="acc_type"> Checking <input type="radio" name="acc_type"> Saving </td>
						<td></td><td style="text-align:left;vertical-align:top;padding:0"><input type="submit" name="pay_submit" id="pay_submit" class="button_submit" value="Pay">
						</td></tr>
						
						<tr><td style="text-align:left;vertical-align:top;padding:0"> Account Number:</td>
						<td style="text-align:left;vertical-align:top;padding:0"><input type="text" class="form_style num" name="username_pay" maxlength="10" id="username_pay" ><i class="fa fa-question-circle" style="cursor:pointer;" title="In the Account Number text box, enter the bank account number (from the lower-left portion of a check)."></i>
</td>
						<td style="text-align:left;vertical-align:top;padding:0">Amount:</td>
...
echo "<label style='color:red;'><b>Welcome ".$row_user['CustomerName']." ".$row_user['lastname'].".Please enter new passwork and enter date of birth for security reasons</b></label><br>";}?>
	 ...
	 <div class="modal-body col-md-12">
	  ...
	  <b>Date of Birth1:*</b>
       ...
       <div class="col-md-6 padding_class">
	   <input type="text" readonly="true" name="dob" id="dob" value='<?php echo $row_user['DateOfBirth'];?>' class="form_style"/>
	   <div id="pass_err" style="color:#FF0000;"></div>
	  </div>
