		...
		<div id="content">
			...
			{echo "Transaction success<br>";}
				...
				{ ?> 
					...
					<?php $sql_paid ="SELECT sum(TotalPaid) as paid FROM jgrov_User.tblTransactionDetails where tblSEId = '".$row['Id']."'";
	
							$query_paid = sqlsrv_query($conn, $sql_paid);
							$row_paid = sqlsrv_fetch_array($query_paid);
							$paid=$row_paid['paid'];
							
							$sql_total ="SELECT Totalamt as total FROM jgrov_User.tblTransactionDetails where tblSEId = '".$row['Id']."'";
					...
					<td style="border:thin solid;"><?php if(($total)<=(3*$paid) && $total!=''){echo "Paid";} else{echo "-";}?></td>
					<td style="border:thin solid;"><?php if(($total)<=((3/2)*$paid) && $total!=''){echo "Paid";} else{echo "-";}?></td>
					<td style="border:thin solid;"><?php if(($total-1)<=$paid && $total!=''){echo "Paid";} else{echo "-";}?></td>
				...
				<?php $i++; } ?>
					...
					Synapse Payment
						...
						<?php $i=0;
						while($legal[$i]!=''){?>
						<option value="<?php echo $id1_array[$i];?>"><?php echo $legal[$i];?></option>
						<?php $i++;
						if($i>200){break;}}?>
						</select></td></tr>
