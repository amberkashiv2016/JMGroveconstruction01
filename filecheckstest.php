<?php
	...
	$file_arr = array();
	for($i=0 ; $i<2;$i++){
		$sqlx="SELECT CONVERT(IMAGE ,CONVERT(VARCHAR(MAX) ,CONVERT(VARBINARY(MAX) ,".$db_col[$i]."))) AS  'UploadedFile', Id FROM   tblInstallUsers where  ".$db_col[$i]." !='' AND Id >3680 order by Id desc";
