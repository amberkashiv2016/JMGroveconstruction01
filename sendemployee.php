<?php
	...
	if( $odbccon === false ) {	die( print_r( "can not connect to database..", true)); }
	...
	if ($query === false){
		exit("<pre>".print_r("could not add you at this time..", true));
	}
	else
	{
		...
		$email = $_POST['email'];
		$result = odbc_fetch_array(odbc_exec("select @@IDENTITY as id"));
		$lastID = $result['id'];
		
		$sql="UPDATE tblInstallUsers SET tblInstallUsers.AddedByUserID=1537 WHERE Id=$lastID";
		$query = odbc_exec($sql);
